...
// ICO
//
ico_sot_buy.addEventListener("submit", async e => {
  e.preventDefault();
  const form = e.target;
...
// LP
//
let currentSotToEthPrice = 5;

provider.on("block", n => {
  console.log("New block", n);
  // TODO: Update currentSotToEthPrice
});

lp_deposit.eth.addEventListener("input", e => {
  lp_deposit.sot.value = +e.target.value * currentSotToEthPrice;
});

lp_deposit.sot.addEventListener("input", e => {
  lp_deposit.eth.value = +e.target.value / currentSotToEthPrice;
});
...
lp_deposit.addEventListener("submit", async e => {
  ...
  const form = e.target;
  const eth = ethers.utils.parseEther(form.eth.value);
  const sot = ethers.utils.parseEther(form.sot.value);
  console.log("Depositing", eth, "eth and", sot, "sot");
...
function updateSwapOutLabel() {
  swapOut.value =
    swapIn.type === "eth" ? +swap.amount_in.value * currentSotToEthPrice : +swap.amount_in.value / currentSotToEthPrice;
